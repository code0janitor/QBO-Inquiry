...
import code.janitor.qbo.web.helper.QBOServiceHelper;
...
public class QBOController {
    ...
    public String callQBOCompanyInfo() {
    	...
    	String realmId = (String)TokenContext.getInstance().get("token").getRealmId();
    	if (StringUtils.isEmpty(realmId)) {
    		return new JSONObject().put("response","No realm ID.  QBO calls only work if the accounting scope was passed!").toString();
    	}
    	String accessToken = TokenContext.getInstance().get("token").getAccessToken();
        ...
        try {
	         ...
	         */
	        	...
	        	String refreshToken = TokenContext.getInstance().get("token").getRefreshToken();
	        	
				try {
					BearerTokenResponse bearerTokenResponse = client.refreshToken(refreshToken);
					
			        AuthToken token = TokenContext.getInstance().get("token");
